      ...
      <div className="relative overflow-hidden group">
        <img
          src={
            !property.image
              ? "/placeholder.svg"
              : property.image?.startsWith("http") || property.image?.startsWith("data:") || property.image?.startsWith("blob:")
                ? (property.image.includes("localhost:5000") ? property.image.replace("http://localhost:5000", config.API_URL).replace("https://localhost:5000", config.API_URL) : property.image)
                : property.image?.startsWith("/assets") || property.image?.startsWith("assets")
                  ? (property.image.startsWith("/") ? property.image : `/${property.image}`)
                  : property.image?.startsWith("/uploads")
                    ? `${config.API_URL}${property.image}`
                    : `${config.API_URL}/uploads/${property.image}`
          }
          alt={property.name}
          onError={(e) => {
